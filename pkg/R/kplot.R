...
kplot.ilab<-function(ilab, ...) {
	pars <- list(...)
	if(is.null(pars$x)) pars$x <- ilab$data$x
	if(is.null(pars$labels)) pars$labels <- as.character(ilab$data$org)
	if(is.null(pars$U.lo)) pars$U.lo <- ilab$data$U.lower
	if(is.null(pars$U.hi)) pars$U.hi <- ilab$data$U.upper
	if(is.null(pars$main)) pars$main <- ilab$title[1]
	if(!is.null(pars$do.pdf)) {
		if(pars$do.pdf) {
			if(any(pars$U.lo != pars$U.hi )) {
				warning("do.pdf=TRUE does not support asymetry; using mean U")
				pars$U <- ( ilab$data$U.lower + ilab$data$U.upper )/2
			}
			if(is.null(pars$U)) pars$U <- ilab$data$u * ilab$data$k 
		}
	
	}
	if(!is.na(ilab$subset)) pars$main <- paste(pars$main, "\nSubset: ", ilab$subset, sep="")
