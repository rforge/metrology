...
uncert.default<-function(u, c, method=c("GUM", "MC"), cor, cov, distrib=NULL, 
		distrib.pars=NULL, B=200, x=NULL, keep.x=TRUE, ...) {
...
uncert.function<-function(FUN, x, u, method=c("NUM", "kragten", "k2", "MC"), cor, cov, 
	...
	if(method %in% c("NUM", "kragten", "k2") ) {
		...
		if(method == "NUM") {
			...
			for(i in 1:length(x)) {
				ci[[names(x)[i]]] <- 
					(do.call(FUN, c(x[-i],x.plus[i],...)) - do.call(FUN, c(x[-i],x.minus[i],...)))/(2*u[[i]]*delta)
			}
		}
		...
		if(method == "k2") {
			...
			for(i in 1:length(x)) {
				ci[[names(x)[i]]] <- 
					(do.call(FUN, c(x[-i],x.plus[i],...)) - do.call(FUN, c(x[-i],x.minus[i],...)))/(2*u[[i]])
			}
		}
		...
		if(method == "kragten") {
			...
			for(i in 1:length(x)) {
				ci[[names(x)[i]]] <- 
					(do.call(FUN, c(x[-i],x.plus[i],...)) - y0)/(u[[i]]*sign(delta))
			}
		}
		...
		rv <- .construct.uncert(expr=FUN, y=y0[1], u.y=sqrt(v), x=x, u=u, ci=ci, 
			method=method, call=match.call(),cov=cov, cor=cor, delta=delta,
			cov.xy=cov.xy, cor.xy=cor.xy, ...)
	
		
	} else if(method=="MC") {
		rv<-uncertMC(FUN, x=x, method="MC", 
...
uncert.expression<-function(expr, x, u, method=c("GUM", "NUM", "kragten", "k2", "MC"), cor, cov, 
				distrib=NULL, distrib.pars=NULL, B=200, delta=0.01, keep.x=TRUE, ...) {
	...
	expr.names<-all.vars(expr)
	
	if( any( !(expr.names %in% names(c(x,...))) ) | 
		any( !(names(c(x,...)) %in% expr.names) )) {
	...
	if(method %in% c("NUM", "kragten", "k2") ) {
		...
		body(f.expr)<-expr
		formals(f.expr) <- c(x,...)
		rv<-uncert.function(FUN=f.expr, x=x, u=u, method=method, 
		...
		cor.xy<-cov.xy/(uv*sqrt(v))
		names(cov.xy)<-names(cor.xy)<-names(x)
		rv <- .construct.uncert( expr=expr, y=y[1], u.y=sqrt(v), x=x, u=u, ci=ci, method="GUM", 
				cor=cor, cov=cov, cov.xy=cov.xy, cor.xy=cor.xy, deriv=d.expr, ...)
		...
		rv<-uncertMC(expr, x=x, method="MC", 
			cor=cor, cov=cov, distrib=distrib, distrib.pars=distrib.pars, B=B, keep.x=keep.x,  ...)
			#Does not need u as cov is supplied
		rv$call <- match.call()
	} else {
...
uncert.formula<-function(formula, x, u, method=c("GUM", "NUM", "kragten", "k2", "MC"), cor, cov, 
			distrib=NULL, distrib.pars=NULL, B=200, delta=0.01, keep.x=TRUE, ...) {
	...
	if( any( !(formula.names %in% names(c(x,...))) ) | 
		any( !(names(c(x,...)) %in% formula.names) )) {
		stop("Arguments in formula do not match arguments in x and '...'", call.=TRUE )
	}
	...
	if(method %in% c("NUM", "kragten", "k2") ) {
		f.formula<-function() {}
		if ((le <- length(formula)) > 1) {
			body(f.formula)<-formula[[2]]
		} else stop("Invalid formula in uncertainty call")
		formals(f.formula) <- c(x,...)
		rv<-uncert.function(FUN=f.formula, x=x, u=u, method=method, 
				cor=cor, cov=cov, B=B, delta=delta, ...)
	} else if(method=="GUM") {
		...
		rv <- .construct.uncert( y=y[1], u.y=sqrt(v), x=x, u=u, ci=ci, method="GUM", 
				cor=cor, cov=cov, cov.xy=cov.xy, cor.xy=cor.xy, deriv=d.formula, ...)
		...
		rv<-uncertMC(formula, x=x, method="MC", 
			cor=cor, cov=cov, distrib=distrib, distrib.pars=distrib.pars, B=B, keep.x=keep.x, ...)
			#Does not need u as cov is supplied
		rv$call <- match.call()
	} else {
